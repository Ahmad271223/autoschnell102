...
Reparaturwelle 5 (Review 26.09.2026 abends):
  * Nr. 41: die ZWEITE Pruefung laeuft ueber den Ersatz-Scraper (zwei Quellen);
    ohne Ersatz wie bisher, aber mit Kennzeichen unbestaetigt_einzelquelle
  * Nr. 40: ein geaenderter Preis geht durch speicher.preis_aktualisieren
    (Historie, price_changes/price_reductions) — nie nur current_price
  * Nr. 42: 5-Minuten-Sperre mit Heartbeat statt 20 h; Merker "erledigt fuer
    Tag" in market_config, damit nicht jeder Takt neu beginnt
  * Nr. 37: laeuft als eigene Hintergrundaufgabe je Takt (jobs.worker_forever),
    Zeitlimit je Detailabruf 45 s
  * Nr. 39: rueckstand() zaehlt die wartenden Kandidaten fuer das Monitoring"""
...
from markt import apify, budget, konfig, normalisieren, speicher
...
DETAIL_ZEITLIMIT_S = 45         # Nr. 37: ein Detailabruf darf hoechstens 45 s dauern
SPERRE_S = 300                  # Nr. 42: kurze Sperre mit Heartbeat je Kandidat
RUECKSTAND_ALARM = 500          # Nr. 39: ab so vielen wartenden Kandidaten ein Alarm
...
async def pruefen(db, listing: Dict[str, Any]) -> str:
    ...
    aktuell = await db[LISTINGS].find_one(_schluessel(listing), {"_id": 0, "leer_zaehler": 1, "verification_leer_am": 1,
                                                                "active_state": 1, "current_price": 1}) or {}
    leer_vorher = int(aktuell.get("leer_zaehler") or 0)
    # Nr. 41: zweite Pruefung ueber den Ersatz-Scraper, wenn einer konfiguriert ist
    ersatz = konfig.actor_ersatz()
    actor_name = ersatz if (leer_vorher >= 1 and ersatz) else konfig.actor()
    res = await budget.reservieren(db, konfig.kosten_je_lauf_usd(actor_name, 1))
    if res is None:
        return "unklar"
    ...
    try:
        r = await apify.lauf([url], 1, zeitlimit_s=DETAIL_ZEITLIMIT_S, actor_name=actor_name)
    except apify.ApifyFehler as e:
        gelaufen = e.art in ("zeit", "ausfall", "poll")
        await budget.abrechnen(db, res, (e.usd if e.usd is not None else None) if gelaufen else 0.0, 0, gelaufen=gelaufen)
    ...
    if not items:
        if leer_vorher >= 1:
            ...
            setzen = {"active_state": "confirmed_removed", "confirmed_removed_at": jetzt,
                      "verified_at": jetzt, "leer_zaehler": leer_vorher + 1, "verified_actor": actor_name}
            if actor_name == konfig.actor():
                # Nr. 41: beide leeren Antworten kamen vom selben Scraper — kein zweiter Beleg
                setzen["unbestaetigt_einzelquelle"] = True
            await db[LISTINGS].update_one(_schluessel(listing), {"$set": setzen})
...
def _kandidaten_filter(jetzt) -> Dict[str, Any]:
...
async def kandidaten(db, limit: int):
    ...
    return await db[LISTINGS].find(_kandidaten_filter(konfig.jetzt()),
                                   {"_id": 0, "source": 1, "listing_id": 1, "url": 1, "leer_zaehler": 1})\
        .sort([("leer_zaehler", -1), ("not_seen_since", 1)]).to_list(limit)


async def rueckstand(db) -> int:
    """Nr. 39: wie viele Listings warten gerade auf eine Entfernungspruefung."""
    try:
        return int(await db[LISTINGS].count_documents(_kandidaten_filter(konfig.jetzt())))
    except Exception:  # noqa: BLE001
        return 0


async def taeglich(db) -> Dict[str, Any]:
    """Einmal je Tag bis zu MARKT_ENTFERNUNG_MAX_JE_TAG Listings nachpruefen.
    Nr. 42: Merker 'erledigt fuer Tag' (market_config/entfernung) + 5-Minuten-Sperre, die je
    Kandidat verlaengert wird; stirbt der Prozess, uebernimmt der naechste nach 5 Minuten."""
    if not konfig.entfernung_pruefen() or konfig.entfernung_max_je_tag() <= 0:
        return {"geprueft": 0}
    tag = konfig.heute_tag()
    if (await konfig.merker_lesen(db, konfig.ENTFERNUNG_DOK)).get("tag") == tag:
        return {"geprueft": 0, "erledigt": True}
    from job_lock import acquire, release, verlaengern
    name = f"markt-entfernung-{tag}"
    token = await acquire(db, name, ttl_seconds=SPERRE_S)
    if not token:
        return {"geprueft": 0}
