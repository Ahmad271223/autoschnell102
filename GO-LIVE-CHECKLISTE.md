...
| Restore-Kopien blieben für immer liegen | ✅ | Jeder Restore parkte den bisherigen Stand als `<db>__vorher_<zeitpunkt>` samt Datei-Ordnern — vollständige Kopien personenbezogener Daten, die nie aufgeräumt wurden. `restore_mongo.py` löscht sie jetzt nach erfolgreichem Lauf ab 30 Tagen (`--vorher-aufbewahrung`, jüngster Stand bleibt immer). Test: `test_backup_restore.py::test_13`. |
